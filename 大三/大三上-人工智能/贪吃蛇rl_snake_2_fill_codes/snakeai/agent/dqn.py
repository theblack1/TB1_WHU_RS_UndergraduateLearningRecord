...
class DeepQNetworkAgent(AgentBase):
    ...
    def train(self, env, num_episodes=1000, batch_size=50, discount_factor=0.9, checkpoint_freq=None,
        ...
        with tqdm(range(num_episodes), desc='Training') as tbar:
            for episode in tbar:
                ...
                while not game_over:
                    if np.random.random() < exploration_rate:
                        ...
                        # hint: env.num_actions is the number of actions.
                        ################################# Fill in codes #################################
                        ...
                        # hint: use the model.predict function to get the action.
                        ################################# Fill in codes #################################
                        q = self.model.predict(state)
                        action = np.argmax(q[0])

                        ################################# End filling #################################
                        # C:\Users\li\Desktop\AI实习\rl_snake_2_fill_codes\play.py --interface gui --agent dqn --model --level C:\Users\li\Desktop\AI实习\rl_snake_2_fill_codes\snakeai\levels\10x10-obstacles.json --num-episodes 100
                ...
                summary = '|Loss {:8.4f}|Exploration {:.2f}|' + \
                          'Fruits {:2d}|Timesteps {:4d}|Total Reward {:4d}'
                tbar.set_postfix(states=summary.format(
                    loss, exploration_rate,
